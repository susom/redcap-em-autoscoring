...
foreach (range(1,21) as $i) {
	array_push($required_fields, "bdi_$i");
}
...
$s_total = array_intersect_key($src, array_flip($required_fields));

# Calculate number of fields with values
$i = 0;
foreach ($s_total as $k => $v) {
	if ($v != '') {
		$i++;
...
$s_percent_complete = round($i / count($s_total) * 100,2) ;
